...
def test_event_matching_commit_status(commit_status_hook_success_data):
    ...
    assert type(event.source).__name__ == 'Commit'
    assert event.source.repository == commit_status_hook_success_data['repository']['name']
    assert event.source.ssh_url == commit_status_hook_success_data['repository']['ssh_url']
    assert event.source.sha == commit_status_hook_success_data['commit']['sha']

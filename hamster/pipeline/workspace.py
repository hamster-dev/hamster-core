...
class Workspace(contextlib.ContextDecorator):
    """Workspace.
    ...
    def get_acquisition_instructions(self, source):
        #TODO rewrite this whole shit
        # only works for pullrequest obj
        #pytest.set_trace()
        return {
            # clone 'base' repo (pull destination)
            # fetch, then checkout the pull request
            # merge with the base branch
            'command': 'git clone {} {}'.format(source._json_data['base']['repo']['ssh_url'], source.repository[1]),
            'directory': source.repository[1],
            'post_commands': [
                'git fetch origin refs/pull/{}/head'.format(source.number),
                'git checkout -b pr/{} FETCH_HEAD'.format(source.number),
                'git config user.email hamster@nowhere',  # fscking needy merge cmd
                ...
                'git merge --no-commit origin/{}'.format(source.base.ref)
            ]
        }
    ...
    def _acquire_source(self):
        ...
        #    return
        #pytest.set_trace()
        instructions = self.get_acquisition_instructions(self.source)

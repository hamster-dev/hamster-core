...
def make_test_app(broker=None):
    """Factory function for a celery app that
    optionally is connected to a broker, driven
    by project configuration.
    To use a broker:
        `export PIPELINE_TEST_USE_BROKER=redis://localhost/0`
    """
    if not broker:
        try:
            broker = os.environ['PIPELINE_TEST_USE_BROKER']
        except KeyError:
            broker = ''
            eager = True
        else:

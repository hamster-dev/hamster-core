...
def refresh_session(obj):
    """Refresh the github session for an object.
    Needed because these dont serialize well across workers.
    TODO: fix the serialization.
    """
    obj.session = github().session


def github(gh_url=None, gh_user=None, gh_token=None):
    ...
    authorization error.
    """
    if not gh_url and (not hasattr(settings, 'HAMSTER_GITHUB_URL') \
        ...
        TheHub = partial(
            GitHubEnterprise,
            gh_url if gh_url else settings.HAMSTER_GITHUB_URL,
            verify=False  # not sure if we need to verify SSL anymore
        )
    try:
        username = gh_user or settings.HAMSTER_GITHUB_USER
        gh = TheHub(
            username=username,
            token=gh_token or settings.HAMSTER_GITHUB_TOKEN,
        )
        gh.me()  # force a connect attempt, which will raise 401 if login failure
    except GitHubError as ex:
        logger.error("error connecting to github {} as {}: {}".format(TheHub, username, str(ex)))
        raise
    else:
        return gh
